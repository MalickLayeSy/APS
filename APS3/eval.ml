...
and eval_cmds (env,mem,flux) cmds =
  match cmds with
      (Def d)::cs -> eval_cmds (eval_def (env, mem,flux) d) cs
...
and eval_def (env, mem,flux) def =
  match def with
      ConstDef (x, _, e) -> let (v,mem',flux') = eval_sexpr (env, mem,flux) e in ((x, v)::env,mem',flux')
    | FunDef (x, _, args, e) -> ((x,Closure(e,eval_args env args))::env,mem,flux)
    | FunRecDef (x, _, args, e) -> ((x, RecClosure(fun f -> (e, fun a -> (x,f)::(List.combine (List.map get_id_arg args) a)@env)))::env,mem,flux)
    | VarDef (x,_) -> let (a, memRes) = alloc_memory mem in ((x, a)::env, memRes,flux)
    | ProcDef (x, args, bk) -> ((x, ProcClosure(bk, eval_args env args))::env, mem,flux)
    | ProcRecDef (x, args, bk) -> ((x, RecProcClosure(fun p -> (bk, fun a -> (x,p)::(List.combine (List.map get_id_arg args) a)@env)))::env,mem,flux)
    | FunPDef (x, _,args, bk) -> ((x, ProcClosure(bk, eval_args env args))::env, mem,flux)
    | FunRecPDef (x,_,args, bk) -> ((x, RecProcClosure(fun p -> (bk, fun a -> (x,p)::(List.combine (List.map get_id_arg args) a)@env)))::env,mem,flux)
...
and eval_op (env,mem,flux) (op,vals) = 
  match (op,vals) with
    ...
    | ("vset", [val1;val2;val3]) -> let(val1', mem', flux') = eval_sexpr (env,mem,flux) val1    in 
                                    let(val2',mem'', flux')  = eval_sexpr (env,mem',flux') val2 in
                                    let(val3',mem''', flux')  = eval_sexpr (env,mem,flux') val3 in
                                    ( match val1',val2',val3' with
                                    (Block a , Valeur i , v) -> let memo = (editInMem mem''' (a+i) (Some v)) in (Block (a),memo,flux'')
