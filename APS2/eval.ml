...
and eval_sexpr (env, mem) value =
  match value with
    ...
    match (op,vals) with
      ...
      | ("vset", [val1;val2;val3]) -> let(val1', mem') = eval_sexpr (env,mem) val1    in 
                                      let(val2',mem'')  = eval_sexpr (env, mem') val2 in
                                      let(val3',mem''')  = eval_sexpr (env, mem') val3 in
                                      ( match val1',val2',val3' with
                                      (Block (adr, n), Value i , v) -> let mem3 = (editInMem mem''' (adr+i) (Some v)) in ((Block (adr, n)),mem3)
